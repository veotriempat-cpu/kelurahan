...
import { signInAnonymously, onAuthStateChanged, signInWithCustomToken, signInWithEmailAndPassword, signOut } from 'firebase/auth';
        ...
        createApp({
            ...
            setup() {
                ...
                const handleLogin = async () => {
                    ...
                    if (!emailInput || !passInput) return showToast('Harap ketik Email/Username/NIK dan Password Anda!', 'danger');

                    // LOGIN ADMIN UTAMA — Firebase Authentication + root collection /admins/{UID}.
                    // UID tidak perlu ditulis di source code. UID akun Firebase yang berhasil login
                    // dipakai sebagai document ID untuk memeriksa hak akses admin.
                    try {
                        const credential = await signInWithEmailAndPassword(auth, emailInput, passInput);
                        ...
                        if (adminSnap.exists()) {
                            ...
                            const aktif = admin.aktif !== false;

                            if (aktif && (role === 'admin' || role === 'admin_kelurahan' || admin.isAdmin === true)) {
                                user.value = {
                                    nama: admin.nama || credential.user.displayName || 'Administrator',
                                    role: 'admin_kelurahan',
                                    rt: '',
                                    rw: '',
                                    username: admin.username || credential.user.email || '',
                                    ttd: admin.ttd || '',
                                ...
                                return showToast('Login Admin Utama Berhasil');
                            }
                        }

                        // Akun Firebase valid tetapi bukan admin yang terdaftar.
                        await signOut(auth);
                    } catch (e) {
                        ...
                        if (code === 'auth/invalid-credential' || code === 'auth/invalid-login-credentials' || code === 'auth/user-not-found' || code === 'auth/wrong-password') {
                            // Lanjutkan ke autentikasi legacy untuk RT/RW/Babinsa/Bendahara/Warga.
                        } else if (code === 'auth/too-many-requests') {
                            return showToast('Terlalu banyak percobaan login. Silakan coba lagi beberapa saat.', 'danger');
                        } else if (code) {
                            console.error('Firebase Admin Login Error:', e);
                        }
                    }
                    
                    // Login Admin LAMA sengaja tidak lagi diterima.
                    // Admin sekarang WAJIB menggunakan Firebase Authentication + /admins/{UID}.
                ...
                const logout = async () => { try { await signOut(auth); } catch(e) { console.error(e); } isLoggedIn.value=false; currentAuthView.value='landing'; menu.value='dashboard'; user.value={ nama:'', role:'', rt:'', rw:'', username:'', ttd:'' }; showToast('Berhasil Logout'); };
